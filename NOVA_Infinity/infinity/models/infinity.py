...
from infinity.models.nova_basic import novaVARCrossAttnBlock
...
class Infinity(nn.Module):
    def __init__(
        ...
        head_depth=1,
        top_p=0.0, top_k=0.0,
        customized_flash_attn=False, fused_mlp=True, fused_norm=False,
        block_chunks=1,
        checkpointing=None,
        pad_to_multiplier=0,
        use_flex_attn=False,
        ...
        # self.residual_cache = CrossScaleResidualCache(embed_dim=self.C)
        for block_idx in range(depth):
            block = (novaVARCrossAttnBlock if self.t2i else SelfAttnBlock)(
                ...
                # block_idx=block_idx, residual_cache_manager=self.residual_cache
            )
    ...
    def forward(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC_wo_prefix: torch.Tensor, scale_schedule: List[Tuple[int]],
        ...
        **kwargs,
        ...
        # [2. block loop]
        SelfAttnBlock.forward, novaVARCrossAttnBlock.forward
    ...
    def autoregressive_infer_cfg(
        ...
        if inference_mode:
            for b in self.unregistered_blocks: (b.sa if isinstance(b, novaVARCrossAttnBlock) else b.attn).kv_caching(True)
        else:
            assert self.num_block_chunks > 1
            for block_chunk_ in self.block_chunks:
                for module in block_chunk_.module.module:
                    (module.sa if isinstance(module, novaVARCrossAttnBlock) else module.attn).kv_caching(True)
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            ...
            for block_idx, b in enumerate(self.block_chunks):
                ...
                for m in b.module:
                    last_stage, attn_e, ca_e, fnn_e = m(x=last_stage, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=None, attn_fn=attn_fn, scale_schedule=scale_schedule, rope2d_freqs_grid=self.rope2d_freqs_grid, scale_ind=si, layer_idx=layer_idx,x_shape=pn, all_entropy=all_entropy, 
                        previous_entropy_mean_attn=previous_entropy_mean_attn,
                        previous_entropy_mean_ca=previous_entropy_mean_ca,
                        previous_entropy_mean_fnn=previous_entropy_mean_fnn,
                    )
                    ...
                    layer_idx += 1
                    previous_entropy_mean_attn=attn_e
                    previous_entropy_mean_ca=ca_e
                    previous_entropy_mean_fnn=fnn_e
            ...
            # # print(f'logits -1 {logits_BlV[0, 0, :]}')
            # print("--------------------------------")
            
            if self.use_bit_label:
                tmp_bs, tmp_seq_len = logits_BlV.shape[:2]
                logits_BlV = logits_BlV.reshape(tmp_bs, -1, 2)
                H_proxy = token_entropy_from_pairs(logits_BlV, L=tmp_seq_len, reduce="sum", base="2") 
                all_entropy.append(H_proxy)
                # print("--------------------------------")
                # print(H_proxy)
                # print(f'logits_BlV.shape: {logits_BlV.shape}')
                # print("--------------------------------")
                # print(sample_with_top_k_top_p_also_inplace_modifying_logits_(logits_BlV, rng=rng, top_k=top_k or self.top_k, top_p=top_p or self.top_p, num_samples=1).shape)
        ...
        if inference_mode:
            for b in self.unregistered_blocks: (b.sa if isinstance(b, novaVARCrossAttnBlock) else b.attn).kv_caching(False)
        else:
            assert self.num_block_chunks > 1
            for block_chunk_ in self.block_chunks:
                for module in block_chunk_.module.module:
                    (module.sa if isinstance(module, novaVARCrossAttnBlock) else module.attn).kv_caching(False)
        ...
        return ret, idx_Bl_list, img, all_entropy
    ...
    def special_init(
        ...
        for block_idx, sab in enumerate(self.unregistered_blocks):
            sab: Union[SelfAttnBlock, novaVARCrossAttnBlock]
            # init proj
...
def token_entropy_from_pairs(logits_pair: torch.Tensor, L: int, reduce: str = "sum", base: str = "e") -> torch.Tensor:
    """
    logits_pair: (B, L*(V/2), 2) 
    L: 原始序列长度
    reduce: "sum" 或 "mean"，决定把 V/2 个pair的熵相加还是取均值
    base: "e" -> nats, "2" -> bits
    
    return: (B, L)  每个位置的“pair 熵”聚合结果（作为 token 熵的 proxy）
    """
    B, l, two = logits_pair.shape
    assert two == 2, "last dim must be 2 (pair)"
    assert l % L == 0, f"l = {l} must be divisible by L = {L}"
    num_pairs = l // L  # 例如 V=64 -> num_pairs=32

    # (B, L*(V/2), 2) -> 概率
    probs = F.softmax(logits_pair, dim=-1).clamp_min(1e-12)

    # 切换对数底
    if base == "2":
        logp = probs.log2()
    else:
        logp = probs.log()  # 自然底 (nats)

    # pair 熵: (B, L*(V/2))
    H_pair = -(probs * logp).sum(dim=-1)

    # 还原到 (B, L, V/2)
    H_pair = H_pair.view(B, L, num_pairs)

    # 聚合成 (B, L)
    if reduce == "mean":
        H_token_like = H_pair.mean(dim=-1)
    else:
        H_token_like = H_pair.sum(dim=-1)
