...
from hart.modules.networks.nova_utils import compute_merge


def compute_relative_pruning_ratio(
    scale_ind,
    start_ind,
    entropy_diff,
    transition_lag: float = 1.0,
    temperature: float = 0.8,
    entropy_sensitivity: float = 0.1,
):
    """Match Infinity's pruning schedule: a sigmoid ramp after `start_ind`, damped by entropy."""
    ...
    e_val = entropy_diff.item() if isinstance(entropy_diff, torch.Tensor) else float(entropy_diff)
    damping = entropy_sensitivity * math.tanh(max(0.0, e_val))
...
def nova_apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=2, idx_fn=None):
...
class NOVALlamaAttention(nn.Module):
    def __init__(
        self,
        block_idx,
        patch_nums,
    ...
    def forward(
        ...
        q, k = nova_apply_rotary_pos_emb(q, k, cos, sin, unsqueeze_dim=dim_unsqueeze,idx_fn=idx_fn)
...
class NOVAAdaLNSelfAttn(nn.Module):
    def __init__(
        self,
        block_idx,
        last_drop_p,
        ...
        # ---- NOVA / Infinity-aligned pruning & cache state (used when disable_aln=True) ----
        self.previous_scale_cache_self_attn = None
        self.previous_scale_cache_ffn = None

        # Scale labels used by Infinity's 10-stage schedule (kept here for consistent behavior)
        self.label_cache = [1, 3, 4, 6, 8, 12, 16, 20, 24, 32, 40, 48, 60]

        # When entropy becomes stable, we start caching from one scale and prune subsequent scales
        self.cached_size = []   # list of scale labels to cache features for later restoration
        self.purne_scale = []   # list of scale labels to prune (skip tokens)
    ...
    def forward(
        ...
        scale_ind: int = 0,
        all_entropy=None,
        previous_entropy_mean_attn: float = 0.0,
        previous_entropy_mean_ffn: float = 0.0,
        ...
        if not self.disable_aln:
            ...
            # ------------------------------------------------------------------
            # Infinity-aligned dynamic pruning & cache reuse (training-free)
            # ------------------------------------------------------------------
            if scale_ind == 0:
                self._reset()
            ...
            if scale_schedule is not None and scale_ind < len(scale_schedule):
                cur_scale = scale_schedule[scale_ind]

            # Decide when to start caching/pruning based on entropy stabilization (same logic as Infinity)
            if all_entropy is not None:
                if (self.entropy_threshold == -1) and (scale_ind > 6):
                    entropy = (
                        torch.abs(torch.mean(all_entropy[scale_ind - 1].view(-1).float()) - torch.mean(all_entropy[scale_ind - 2].view(-1).float()))
                        + torch.abs(torch.mean(all_entropy[scale_ind - 2].view(-1).float()) - torch.mean(all_entropy[scale_ind - 3].view(-1).float()))
                    )
                    if entropy < 1.2:
            ...
            # Smoothly increase pruning ratio after start_ind
            if scale_ind > self.start_ind:
                puring_ratio = compute_relative_pruning_ratio(scale_ind, self.start_ind, entropy, 3.0, 0.8, 0.1)

            # A simple proxy of "later layers" used in Infinity (caller should pass layer_idx)
            is_later_layer = True if (layer_idx is None or layer_idx >= 3) else False

            # ---- Self-Attention path ----
            merge_fn, unmerge_fn, idx_fn, _ = compute_merge(
                x,
                prune_scale_list=self.purne_scale,
                is_later_layer=is_later_layer,
                x_shape=x_shape,
                entropy=entropy,
                cur_scale=cur_scale,
                puring_ratio=puring_ratio,
                previous_e_mean=previous_entropy_mean_attn,
            )

            attn_out = self.attn(
                self.ln_wo_grad(merge_fn(x)),
                attn_bias=attn_bias,
                context_position_ids=context_position_ids,
                context_mask=context_mask,
                si=si,
            ...
            attn_out = unmerge_fn(attn_out, self.previous_scale_cache_self_attn, self.now_cached_size)
            if x.shape[1] in [s * s for s in self.cached_size]:
                self.previous_scale_cache_self_attn = attn_out
            ...
            merge_fn, unmerge_fn, idx_fn, _ = compute_merge(
                ...
                cur_scale=cur_scale,
                puring_ratio=puring_ratio,
                previous_e_mean=previous_entropy_mean_ffn,
            )

            ffn_out = self.ffn(self.ln_wo_grad(merge_fn(x)))
            ffn_out = unmerge_fn(ffn_out, self.previous_scale_cache_ffn, self.now_cached_size)
            if x.shape[1] in [s * s for s in self.cached_size]:
